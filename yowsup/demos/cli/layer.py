...
import time
import random
from yowsup.demos.cli.servicio import LlamarServicio
...
class YowsupCliLayer(Cli, YowInterfaceLayer):
    ...
    def onMessage(self, message):
        messageOut = ""
        self.output("El mensaje es tipo: "+ message.getType(), tag = None, prompt = not self.sendReceipts)
        ...
        output = self.__class__.MESSAGE_FORMAT.format(
            ...
            MESSAGE = messageOut.encode('utf-8') if sys.version_info >= (3, 0) else messageOut.encode("utf-8"),
            MESSAGE_ID = message.getId()
            )
        #Esta es la parte del servicio WEB del chatbot
        servicioob = LlamarServicio(messageOut.encode('utf-8'), message.getFrom(False))
        #self.output(servicioob.respuesta, tag = None, prompt = not self.sendReceipts)
        self.output("Desde el numero: "+message.getFrom(False), tag = None, prompt = not self.sendReceipts)
        tamanocadena = len(servicioob.respuesta)
        #Comparamos la cadena para saber cuanto tiempo debe esperar el mensaje para enviarse correctamente (Simular que es un ser humano) para mas
        #Informacion http://www.dailymail.co.uk/sciencetech/article-3342394/How-fast-type-test-reveal-characters-words-manage-one-minute.html

        tiempo = 0 #variable que almacena el tiempo que debe esperar (esta en segundos)
        tamanocadenamenor = 138
        tamanocadenamayor = 821
        
        if tamanocadena > 0 and tamanocadena < 15:
            tiempo = 5
        elif tamanocadena > 0 and tamanocadena < 50:
            tiempo = 10
        elif tamanocadena > 0 and tamanocadena < 75:
            tiempo = 15
        elif tamanocadena > 0 and tamanocadena < 100:
            tiempo = 20
        elif tamanocadena > 0 and tamanocadena <  tamanocadenamenor + 1:
            tiempo = 30
        elif tamanocadena > 0 and tamanocadena < 269:
            tiempo = 40
        elif tamanocadena > 0 and tamanocadena < 358:
            tiempo = 50
        elif tamanocadena > 0 and tamanocadena < 450:
            tiempo = 53
        elif tamanocadena > 0 and tamanocadena < 533:
            tiempo = 53
        elif tamanocadena > 0 and tamanocadena < 669:
            tiempo = 53
        elif tamanocadena > 0 and tamanocadena < tamanocadenamayor + 1 :
            tiempo = 53
        else:
            tiempo = 53
        self.output("Tiempo espera: "+str(tiempo) +" Para la cadena: "+str(tamanocadena) +" Fecha "+time.strftime("%H:%M:%S"), tag = None, prompt = not self.sendReceipts)
        time.sleep(tiempo)        
        self.message_send(message.getFrom(False),servicioob.respuesta)
        if self.sendReceipts:               
